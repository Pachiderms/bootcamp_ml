...
def final_train():
    ...
    plt.scatter(time_delivery, prod_distance, c=predictions[1], marker='x', s=40, alpha=0.4)
    

def final_train_reg():
    (x_train, x_test, y_train, y_test) = split_set()
    xtr, xtst = add_polynomial_features(x_train, 2), add_polynomial_features(x_test, 2)
    best_model = MyRidge(np.ones(xtr.shape[1] + 1), alpha=5e-5, max_iter=50000, lambda_=0.2).fit_(xtr, y_train)
    y_hat = best_model.predict_(xtst)

    models, errors, predictions = train_models_reg_from_csv(x_train, x_test, y_train, y_test, [1, 3, 4])
