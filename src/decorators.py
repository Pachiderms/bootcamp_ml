...
def check_type_and_shape_reg_gradient(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if not isinstance(y, np.ndarray) or not isinstance(x, np.ndarray) \
            ...
            _debug_print(func.__name__, "type", y=type(y), x=type(x), theta=type(theta), lambda_=type(lambda_))
            return None
        m, n = x.shape
        if y.shape != (m, 1) or theta.shape != (n + 1, 1):
            _debug_print(func.__name__, "shape", x=x.shape, y=y.shape, theta=theta.shape)
            return None
...
def check_type_and_shape_fit(func):
    ...
    def wrapper(x, y, theta, alpha, *args, **kwargs):
        if not isinstance(x, np.ndarray) or not isinstance(y, np.ndarray) \
            ...
            _debug_print(func.__name__, "type", x=type(x), y=type(y), theta=type(theta), alpha=type(alpha))
            return None
        m, n = x.shape
        if y.shape != (m, 1) or theta.shape != (n + 1, 1):
            _debug_print(func.__name__, "shape", x=x.shape, y=y.shape, theta=theta.shape)
...
def check_type_and_shape_reg_loss(func):
    ...
    def wrapper(y, y_hat, theta, lambda_, *args, **kwargs):
        if not isinstance(y, np.ndarray) or not isinstance(y_hat, np.ndarray) \
            or not isinstance(theta, np.ndarray) or not isinstance(lambda_, float):
            _debug_print(func.__name__, "type", y=type(y), y_hat=type(y_hat), theta=type(theta), lambda_=type(lambda_))
            return None
        if y.shape != y_hat.shape or y.shape[1] != 1 or theta.shape[1] != 1:
            _debug_print(func.__name__, "shape", y=y.shape, y_hat=y_hat.shape, theta=theta.shape)
            return None
...
def check_type_and_shape_fit_method(func):
    ...
    def wrapper(self, x, y, *args, **kwargs):
        ...
        if not isinstance(x, np.ndarray) or not isinstance(y, np.ndarray) \
            or not isinstance(theta, np.ndarray) or not isinstance(alpha, (float, int)):
            _debug_print(func.__name__, "type", x=type(x), y=type(y), theta=type(theta), alpha=type(alpha))
            return None
        m, n = x.shape
        if y.shape != (m, 1) or theta.shape != (n + 1, 1):
