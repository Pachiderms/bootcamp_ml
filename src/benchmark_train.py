...
def train_models(x_train, x_test, y_train, y_test, degree):
    ...
    for d in range(1, degree + 1):
        if d == 1:
            ...
            xtr, xtst = add_polynomial_features_mult(x_train, d), add_polynomial_features_mult(x_test, d)

        m = MyLR(thetas=np.ones(xtr.shape[1] + 1), alpha=5e-5, max_iter=50000).fit_(xtr, y_train)
        models.append(m)
...
def train_models_from_csv(x_train, x_test, y_train, y_test, degrees, file="../attachments/models.csv"):
    df = pd.read_csv(file)
...
def train_models_reg(x_train, x_test, y_train, y_test, degree):
    ...
    for d in range(1, degree + 1):
        ...
        for lambda_ in lambdas_:
            m = MyRidge(thetas=np.ones(xtr.shape[1] + 1), alpha=5e-5, max_iter=50000, lambda_=lambda_).fit_(xtr, y_train)
            models.append(m)
            degrees.append(d)
            y_hat = m.predict_(xtst)
            predictions.append(y_hat)
...
def train_models_reg_from_csv(x_train, x_test, y_train, y_test, degrees, file="../attachments/models_reg.csv"):
    df = pd.read_csv(file)
    df['thetas'] = df["thetas"].apply(lambda x: np.array(ast.literal_eval(x)))
    df = df[df['degree'].isin(degrees)]
    deg = df['degree']
    ...
    for d in range(1, len(degrees) + 1):
            ...
            if degree == 1:
                ...
                xtr, xtst = add_polynomial_features(x_train, degree), add_polynomial_features(x_test, degree)

            m = MyRidge(thetas=thetas.iloc[d - 1], alpha=alphas.iloc[d - 1], max_iter=max_iters.iloc[d - 1], lambda_=lambdas_.iloc[d - 1]).fit_(xtr, y_train)
            models.append(m)
            y_hat = m.predict_(xtst)
            predictions.append(y_hat)
            errors.append(m.mse_(y_test, y_hat))
